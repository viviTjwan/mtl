...
class MTLModel(BaseModel):
  ...
  def __init__(self, all_data, adv, is_train):
    # input data
  ...
  def build_task_graph(self, data):
    ...
    sentence = tf.reshape(sentence, [-1, 10, 16])
    if self.is_train:
    ...
    # todo:!!!!
    loss_adv, loss_adv_l2 = self.adversarial_loss(feature, task_label)
    loss_diff = self.diff_loss(feature, feature)
    ...
    with tf.Session() as sess:
      labels_numpy = labels.eval()
      pred_numpy = labels.eval()
    ...
    self.tensors.append((acc, loss, f1))
...
def build_train_valid_model(model_name, all_train, all_test, adv, test):
  with tf.name_scope("Train"):
    with tf.variable_scope(model_name, reuse=None):
      m_train = MTLModel(all_train, adv, is_train=True)
      m_train.build_train_op()
      m_train.set_saver(model_name)
  with tf.name_scope('Valid'):
    with tf.variable_scope(model_name, reuse=True):
      m_valid = MTLModel(all_test, adv, is_train=False)
